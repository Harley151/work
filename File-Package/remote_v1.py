...
if __name__ == '__main__':
    ...
    try:
        with open(files, 'r', encoding='utf-8') as file:
            ...
            for line in lines:
                a += line.strip()
            Iplist = ast.literal_eval(a)
    except FileNotFoundError:
        print('\33[0;32m1. 通过 -i 指定远程文件,如下:\33[0m')
        print('--> remote -i "/root/iplist.txt" "ls"')
        print()
        print('\33[0;32m2. 在/sun/放置远程文件iplist.txt,如下:\33[0m')
        print('--> remote "ls"')
        sys.exit()
    ...
    def Linux(command):
        ...
        for k, v in Iplist.items():
            linux = SSH_Executive(k, 22, "root", v, command, num)
